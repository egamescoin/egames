...
#include "zegamescoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zEGAMES ought to be enough for anybody." - Bill Gates, 2017
    ui->zEGAMESpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzEGAMESSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzEGAMES->setEnabled(false);
        ui->pushButtonMintzEGAMES->setToolTip(tr("zEGAMES is currently disabled due to maintenance."));

        ui->pushButtonSpendzEGAMES->setEnabled(false);
        ui->pushButtonSpendzEGAMES->setToolTip(tr("zEGAMES is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zEGAMESpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzEGAMES_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEGAMES is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zEGAMES...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zEGAMES in ") + 
...
void PrivacyDialog::on_pushButtonSpendzEGAMES_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEGAMES is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zEGAMES
        sendzEGAMES();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zEGAMES
    sendzEGAMES();
}

void PrivacyDialog::on_pushButtonZeGamesControl_clicked()
{
    ZeGamesControlDialog* zeGamesControl = new ZeGamesControlDialog(this);
    zeGamesControl->setModel(walletModel);
    zeGamesControl->exec();
}

void PrivacyDialog::setZeGamesControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzeGamesSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzEGAMES()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid eGames Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zEGAMESpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zEGAMES is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " EGAMES </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zEGAMES</b>";
    ...
    // use mints from zeGames selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZeGamesControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZeGamesControlDialog::GetSelectedMints();
    }
    ...
    // Clear zegamescoin selector in case it was used
    ZeGamesControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zeGames Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " eGames, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zeGames Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEGAMES </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zEGAMES "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zEGAMES "));
    ui->labelzEGAMESAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzEGAMESSyncStatus->setVisible(fShow);
}
