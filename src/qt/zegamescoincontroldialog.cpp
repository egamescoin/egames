...
#include "ui_zegamescoincontroldialog.h"
...
std::list<std::string> ZeGamesControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZeGamesControlDialog::listMints;

ZeGamesControlDialog::ZeGamesControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZeGamesControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZeGamesControlDialog::~ZeGamesControlDialog()
{
    delete ui;
}

void ZeGamesControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZeGamesControlDialog::updateList()
{
...
void ZeGamesControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZeGamesControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZeGames_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZeGamesControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZeGamesControlDialog::GetSelectedMints()
{
...
void ZeGamesControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
